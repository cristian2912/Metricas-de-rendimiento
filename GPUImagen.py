...
# warm-up 
img_gpu_w = cp.asarray(img, dtype=cp.uint8).ravel()
gray_gpu_w = cp.zeros(w*h, dtype=cp.uint8)
blocks = ((w + 15)//16, (h + 15)//16)
threads = (16, 16)
rgb_to_gray_kernel(blocks, threads, (img_gpu_w, gray_gpu_w, w, h))
...
print(f"CSV: algo=GPU,shape={w}x{h},total_s={total:.6f},gray_s={t_gray:.6f},edges={edge_pixels}")
